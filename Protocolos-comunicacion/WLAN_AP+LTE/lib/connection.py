...
class WiFi:
    ...
    def set_antenna(self, antenna):
        self.wlan.antenna(antenna)

    def set_mode(self, mode):
        self.mode = mode

    def config_STA(self, ssid, password, antenna, mode):
        self.wlan.init(mode=self.mode)
    ...
    def config_AP(self):
        self.wlan.init(mode=WLAN.AP, ssid=self.ssid, auth=(WLAN.WPA2, self.password), channel=7, antenna=WLAN.INT_ANT)
        print('WLAN AP connection succeeded!')